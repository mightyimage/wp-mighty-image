...
class MightyImageHelper {
  ...
  static public function url_format($url = '', $params = array() , $skip_empty = false, $separator = '&') {
    if ($url != '') {
      ...
      $query = MightyImageHelper::url_query($params);
  ...
  */
  ...
  static public function is_wpmu() {
    ...
    if ($wpmu === null) {
      $wpmu = (file_exists(ABSPATH . 'wpmu-settings.php') || (defined('MULTISITE') && MULTISITE) || defined('SUNRISE') || MightyImageHelper::is_wpmu_subdomain());
    }
  ...
  static public function get_url_regexp($url) {
    ...
    $regexp = '(https?:)?//(www\.)?' . MightyImageHelper::preg_quote($url);
  ...
  static public function document_root() {
    ...
    if (!empty($_SERVER['SCRIPT_FILENAME']) && !empty($_SERVER['PHP_SELF'])) {
      ...
      $php_self = MightyImageHelper::normalize_path($_SERVER['PHP_SELF']);
  ...
  static public function url_to_docroot_filename($url) {
    ...
    $normalized_url = MightyImageHelper::remove_query_all($normalized_url);
  ...
  static public function url_relative_to_full($relative_url) {
    ...
    if (version_compare(PHP_VERSION, '5.4.7') < 0) {
      if (substr($relative_url, 0, 2) == '//') {
        $relative_url = (MightyImageHelper::is_https() ? 'https' : 'http') . ':' . $relative_url;
      }
    }
