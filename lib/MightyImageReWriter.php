...
class MightyImageReWriter
{
  ...
  private function fill_regexps()
  {
    ...
    if ($domain_url_regexp != MightyImageHelper::get_url_regexp(MightyImageHelper::url_to_host(site_url())))
      $site_domain_url_regexp = MightyImageHelper::get_url_regexp(MightyImageHelper::url_to_host(site_url()));
    ...
    if ($mask != '') {
      ...
      if ($site_domain_url_regexp)
        $regexps[] = '~(["\'(=])\s*((' . $site_domain_url_regexp . ')?(' . MightyImageHelper::preg_quote($site_path . WPINC) . '/(' . MightyImageHelper::get_regexp_by_mask($mask) . ')([^"\'() >]*)))~i';
    ...
    if (count($masks)) {
      ...
      if (count($custom_regexps_docroot_related) > 0) {
        ...
        if ($site_domain_url_regexp)
          $regexps[] = '~(["\'(=])\s*((' . $site_domain_url_regexp . ')?(' . MightyImageHelper::preg_quote($site_path) . '(' . implode('|', $custom_regexps_docroot_related) . ')([^"\'() >]*)))~i';
      }
    }
